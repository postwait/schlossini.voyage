...
var reverse_use = function(app) {
  ...
  passport.use(new GoogleStrategy({
    ...
    function(request, accessToken, refreshToken, profile, done) {
      return done({ id: profile.id, email: profile.email, display_name: profile.displayName,
  ...
  app.get('/auth/smugmug/callback/:method', function(req, res, next) {
    passport.authenticate('smugmug', function(err, user_info, info) {
      ...
      if(method === 'login') {
        perform_login(req, res, { oauth: { service: 'smugmug',
  ...
  app.get('/auth/google/callback', function(req, res, next) {
    passport.authenticate('google', function(info) {
      ...
      if(method === 'signup') {
        ...
        perform_login(req, res, { oauth: { service: 'google',
                                           display_name: info.displayName,
                                           token: info.access_token,
                                           secret: info.refresh_token,
                                           remoteid: info.id } })
...
var login_response = function(req, res, data) {
  ...
  return res.redirect('/login?to=' + encodeURIComponent(req.location || '/'));
}
...
function perform_login(req, res, crit) {
  ...
  user.getUser(crit, pass, function(err, users) {
    if(err) err = 'There was a database error, please try again soon!';
    else if(users == null || users.length < 1) err = 'No such user';
    else if(users && users.length > 1) err = 'User parallax';
    ...
    else login_response(req,res,{location: req.body.to || '/'});
  });
}
...
function associate_user(req, res, obj) {
  ...
  user.associateUser(obj, function(err, users) {
    ...
    login_response(req, res, {location: req.body.to || '/'});
  });
}

function create_user(req, res, obj) {
  user.createUser(obj, function(err, users) {
    if(err) return login_response(req,res,{error:err});
